...
| Full-CI worktree | `712fe36a` plus alphabetical `media_tools` module-order fix | `just ci` rerun exited 0; ordering fix and evidence are not yet committed. |
...
not publicly reachable. Full CI also passes on the immediately following worktree after one
mechanical rustfmt ordering fix. No immutable public documentation/release tag has been
verified. A future release must commit the fix/evidence, then add its pushed ref, public
clean-clone proof, and tag rather than rewriting this historical record.
