...
Preview Studio's local library needs no relay configuration. Live sites must be reachable from the desktop. Image/video generation requires its external provider credentials/account or installed CLI. Provider keys entered in Studio use renderer-local storage.
