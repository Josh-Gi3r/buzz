...
The current inherited source baseline is Block Buzz Desktop `v0.5.9` at `ee33722615ca1e7b8efb03e2ed641d99448c8899`. Historical verification refs remain recorded in the detailed QA ledger.
...
| Upstream desktop upgrades | `.github/upstream-desktop-baseline`, `scripts/latest-upstream-desktop-tag.sh`, `scripts/sync-upstream-desktop-local.sh` | `scripts/test-upstream-desktop-sync-contract.sh` checks stable-tag selection and proves the updater contains no push or Actions behavior. |
| Full local quality gate | Root `Justfile`, component test/build configuration | Exact candidate `4e2e785d` passed `just ci`; exact suite counts and ignored boundaries are in QA evidence. |
