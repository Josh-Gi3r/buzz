...
> [implemented BUZZ — LIVE PREVIEW STUDIO architecture](../architecture/preview-studio.md). Current
> artifacts and reviews are localStorage state, not signed relay events.

**Current fork baseline:** published upstream tag `desktop-v0.5.9`
**Scope:** the architectural foundation for an additive BUZZ — LIVE PREVIEW STUDIO product layer on Buzz,
...
5. Keep BUZZ — LIVE PREVIEW STUDIO behind an independently controllable feature vertical.
...
Current BUZZ — LIVE PREVIEW STUDIO state is explicitly device-local.
...
Unknown event kinds are rejected, which means BUZZ — LIVE PREVIEW STUDIO's first-class event model
...
hides network-backed BUZZ — LIVE PREVIEW STUDIO publishing rather than attempting unsupported event kinds.
...
BUZZ — LIVE PREVIEW STUDIO projection tables must not be added to the upstream SQL migration sequence:
...
BUZZ — LIVE PREVIEW STUDIO uses a registry contract rather than a central type switch spread across the
...
BUZZ — LIVE PREVIEW STUDIO, send scoped input events back, reset the environment between sessions, and
...
profile. In v0, Studio tokens are scoped to the BUZZ — LIVE PREVIEW STUDIO screen.
...
This fork includes the published `desktop-v0.5.9` release. Moving upstream proposals
