...
The source tree includes upstream `desktop-v0.5.9`. Exact product, documentation, and
verification commits are separated in the [evidence baseline](../evidence/baseline.md) so
normal setup does not depend on an internal audit ref.
...
- **BUZZ — LIVE PREVIEW STUDIO** is this fork's device-local website/media proofing room.
- **Settings** controls identity, community, appearance, connection, and preview behavior.

Continue with [Communities and chat](communities-and-chat.md), [Working with agents](working-with-agents.md), or the [BUZZ — LIVE PREVIEW STUDIO guide](../preview-studio/user-guide.md).
