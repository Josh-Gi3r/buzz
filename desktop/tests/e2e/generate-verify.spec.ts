...
test("generate panel offers real model choices", async ({ page }) => {
  ...
  const models = await page.getByTestId("preview-studio-model-select").locator("option").allTextContents();
  console.log("models offered:", models.join(" | "));

  // no key stored -> generate is blocked, nothing can fire by accident
  await page.getByTestId("preview-studio-prompt").fill("a calm nordic landing page hero");
  const disabled = await page.getByTestId("preview-studio-generate").isDisabled();
